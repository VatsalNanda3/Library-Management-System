//Config files for admin, user authentication
...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	AuthenticationProvider authenticationProvider()
	{
		...
		provider.setPasswordEncoder(new BCryptPasswordEncoder());//We use the BCrypt password encoder and we have to 
		//store it in the hash code only.In future, when a user is registering, the password will be auto encrypted and would be saved to the database.
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
                ...
                .antMatchers("/userById/{id}").access("hasAuthority('ADMIN')")
                .antMatchers("/userByName/{name}").access("hasAuthority('ADMIN')")
